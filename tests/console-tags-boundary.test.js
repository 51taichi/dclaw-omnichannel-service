...
test("conversation cards expose click and right-click entry points for one manual tag menu", () => {
  ...
  assert.match(css, /\.conversation-metadata-flow\s*\{[\s\S]*flex-flow:\s*row wrap/);
  assert.match(css, /\.flow-session-card \.conversation-metadata-flow\s*\{[\s\S]*grid-column:\s*2 \/ -1/);
