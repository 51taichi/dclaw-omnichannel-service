...
const { createDirectGroupAutomationWorker } = await import(
  "../src/group-automation-direct-worker.js"
);

const targetAt = "2026-08-06T01:00:00.000Z";

function createContext({ botId, taskType = "conditional_push", mention = true }) {
...
function workerFor({ executeAgentTask, sendGroupMessage, now = targetAt }) {
  return createDirectGroupAutomationWorker({
    db: {
