...
- 每个 Bot 可独立开启或关闭夜间自动同步，默认使用北京时间 `03:00-06:00`。
- 夜间自动同步的可配置范围固定为北京时间 `22:00-次日08:00`，不允许设置白天时间。
- 首次开启夜间自动同步或首次执行立即同步时，补同步当前 Bot 全部私聊客户已有标签。
...
管理员首次开启某个 Bot 的夜间自动同步，或在从未回填过时首次点击“立即同步”：
...
首次开启夜间自动同步只登记数据，不立即提交 WorkTool，记录等待下一个维护窗口。首次点击“立即同步”则先完成回填，再由本次人工 run 继续处理待同步记录。
...
- 每个 Bot 使用自己的夜间自动同步开关和维护时间段判断是否允许运行。
- 时区固定为 `Asia/Shanghai`，默认窗口为 `03:00-06:00`。
- 可配置时间被限制在一条标准夜间时间轴：当天 `22:00` 至次日 `08:00`。
- `23:30-04:00` 等跨零点区间有效；`21:00-03:00`、`03:00-10:00` 等包含白天的区间无效。
- 开始时间必须早于标准夜间时间轴上的结束时间，不能配置零时长或反向区间。
- `nightly_enabled` 关闭时，调度器不得创建或恢复 `scheduled` run，但 Outbox 和失败记录保持不变。
...
- 仅当前 Bot 的管理员可以触发。
- `manual` run 不受夜间自动同步开关和维护时间段限制，全天可用。
...
- `夜间自动同步`开关。
- 夜间开始时间，选项限制在北京时间 `22:00-次日08:00`。
- 夜间结束时间，选项限制在北京时间 `22:00-次日08:00`。
- 待同步数量。
- 执行中数量。
- 失败数量。
- 最近一次执行时间和结果。
...
- 首次开启夜间自动同步，或首次立即同步时，提示会登记当前 Bot 全部私聊客户标签。
- 时间组件使用受限选项，不提供可输入任意时间的自由文本框。
- 前端按标准夜间时间轴过滤无效组合，避免选出零时长、反向或包含白天的区间。
- 关闭夜间自动同步后仍展示待同步数量和“立即同步”按钮。
...
- 保存配置时由服务端再次校验 `HH:mm` 格式、北京时间 `22:00-次日08:00` 边界和标准夜间时间轴顺序，不能只依赖前端限制。
- `POST .../run` 只启动或复用当前 Bot 的 manual run，不直接在 HTTP 请求中循环同步。
- `POST .../run` 不受 `nightly_enabled` 限制；从未回填时先登记全量标签，再启动 manual run。
...
- 配置默认值、保存和夜间自动同步开关。
- 接受 `22:00-08:00` 范围内的合法窗口，拒绝包含白天、零时长和反向窗口。
- 首次开启夜间自动同步只补登记私聊客户标签，包含日期标签。
- 从未开启夜间自动同步时首次立即同步也会完成一次全量回填。
...
- 夜间窗口内启动，窗口外不启动。
- 夜间自动同步关闭时不创建 scheduled run，但管理员立即同步仍可运行。
- 窗口内新记录继续处理，窗口结束停止领取。
- 管理员立即同步不受时间窗口限制。
...
- 首次开启夜间自动同步或首次立即同步后，现有私聊客户标签全部进入持久化待同步记录。
- 夜间自动同步只能配置在北京时间 `22:00-次日08:00`，关闭后不再自动运行且不影响立即同步。
