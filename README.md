...
节点激活用于私聊任务状态机：每个节点可以配置“客户多久不回复后再次提醒”、提醒次数、提醒话术，以及是否交给 Agent 美化。默认触发时机为“AI 回复后”，即服务端会在 AI 成功回复客户后创建 SQLite 激活任务；入口节点还可选择“新增好友后”，用于企业微信已自行发送欢迎语或资料链接的场景。WorkTool 回调 `textType=22` 且 `type=105` 时，服务端会从 `friendName` 建立私聊会话并创建入口节点激活任务，不调用 Agent，也不会重复发送欢迎语。
