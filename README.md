...
- 每天、每周、每月或月底按北京时间执行；
- 条件推送：到点后依据指定周期内的完整群消息判断条件，达成才发送用户配置的固定内容；
...
任务状态含义：`倒计时`表示等待下次执行；`执行中`表示本次历史分析或发送尚未结束；`已发送`与`未发送`分别表示本次已经推送，或条件不成立而跳过；`发送待确认`表示外部发送结果不明确，需要人工核对；`执行失败`表示本次分析或明确发送失败；`执行不可用`表示 DClaw 群历史能力、权限或历史回填尚未就绪；`已取消`和`已停用`不会继续自动执行。

群消息先以不可变记录同步到 DClaw 的群历史存储，保留发言人、角色、方向、时间、消息类型和内容。同步由本地已持久化消息唤醒，不调用 Agent，也不使用 WorkTool 群消息轮询；同一消息重复同步会按外部消息 ID 幂等去重。绑定的发布对象和 API Key 必须同时具备 `messages:create`（写入）与 `messages:read`（读取）scope。

任务在目标时间前创建持久化执行记录并冻结任务、群背景和角色配置。短历史一次分析；长历史会转成紧凑文本，按固定字符上限完整分段，再递归合并中间结果，目标时刻只补分析最后一段增量。中间结果只属于本次执行，不形成可复用的业务状态。这样既覆盖完整历史，也避免把整月消息一次性塞入提示词。

群背景和群角色只用于内部理解；对外生成内容不得提及或暗示这些后台配置、内部提示或分析过程。群定时任务与私聊任务状态机、私聊资产完全隔离。条件不成立或分析失败时不发送；周期汇总即使记录稀疏也会生成明确的非空结果。发送前会冻结最终内容和 `@` 名单，WorkTool 明确拒绝时最多重试两次；网络结果不明确时不会自动重发，避免重复触达。

相关 Worker 默认开启，可通过以下环境变量调整本地队列处理频率和租约；这些间隔不是外部消息轮询：

```bash
GROUP_HISTORY_SYNC_WORKER_ENABLED=true
GROUP_HISTORY_SYNC_INTERVAL_MS=2000
GROUP_HISTORY_SYNC_LEASE_MS=120000
GROUP_HISTORY_SYNC_BATCH_SIZE=200
GROUP_HISTORY_SYNC_CLAIM_LIMIT=10
...
升级部署顺序必须是：先部署包含群历史接口的 DClaw，并给对应发布对象和 Key 增加两个消息 scope；再部署本服务并等待所有已管理群完成历史回填；最后才启用群任务执行。旧业务状态表只有在 DClaw 能力验证成功且所有群回填游标追平后才会被事务性、幂等地清理。任一条件未满足时，新群任务保持不可执行，不会回退到旧分析路径。
