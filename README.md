...
同一客户新增好友回调在 10 分钟内会被视为 WorkTool 的重复投递；超过冷却时间后会重新进入入口节点并重新启动首条激活话术。这个重入只重置状态机进度和 AI 接待状态，不会删除聊天记录或已收集的客户资产。可通过 `FRIEND_ADDED_REENTRY_COOLDOWN_MINUTES` 调整冷却时间。
